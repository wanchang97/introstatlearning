...
myDescription <- "The data are a simulated data set containing 1070 purchases where the customer either purchased Citrus Hill or Minute Maid Orange Juice. A number of characteristics of the customer and product are recorded. [@james2013introduction]"
mydataset <- OJ
```
...
# Assignment of Orange Juice Dataset
...
The Orange Juice data set (`OJ`) is available in the `ISLR2` package.
...
A quick exploration can be done with the `skimr` package:
...
```{r}
set.seed(123)
ssize <- nrow(mydataset)
propTrain <- (800/n) #0.8
training.indices <-sample(1:ssize, floor(ssize*propTrain))
train.data  <- mydataset[training.indices, ]
test.data <- mydataset[-training.indices, ]
```

## b. Fit a tree to the training data, with Purchase as the response and the other variables as predictors. Use the summary () function to produce summary statistics about the tree, and describe the results obtained. What is the training error rate? How many terminal nodes does the tree have?
```{r}
library(rpart)
model <- rpart(Purchase ~., data = train.data)
summary(model)
...
print(model)
plot(model)
text(model, digits = 3, cex=0.8)
```
```{r}
library(rpart.plot)
#detach(package:rpart.plot)
rpart.plot(model, cex=.7)

```
## Prediction

```{r}
tree.pred = predict(model,test.data,type="class")
res <- table(tree.pred,test.data$Purchase)
res
```
```{r}
...
```{r}
library(rpart)
model1 <- rpart(Purchase ~., data = mydataset)
# par(xpd = NA) # otherwise on some devices the text is clipped
```
...
**Imagine we know nothing about overfitting**. We may want to check the accuracy of the model on the dataset we have used to build it.

```{r}
predicted.classes<- predict(model1, OJ, "class")
mean(predicted.classes == mydataset$Purchase)
```
...
```{r}
model2 <- rpart(Purchase ~., data = train.data)
predicted.classes.test<- predict(model2, test.data, "class")
mean(predicted.classes.test == test.data$Purchase)
```
...
mydataset$Hig
h=as.factor(ifelse(mydataset$Sales<=8,"No","Yes"))
```
...
```{r}
predicciones <- stats::predict(arbol_regresion, newdata = datos_test)
...
```{r}
predicciones_finales <- stats::predict(arbol_final, newdata = datos_test)
test_rmse    <- sqrt(mean((predicciones - datos_test$medv)^2))
...
```{r}
library(ISLR2)
dataOJ = OJ # Orange Juice Data containing 1070 purchases where the customer either purchases Citrus Hill or Minute Maid Orange Juice
head(dataOJ)
summary(dataOJ)
```
